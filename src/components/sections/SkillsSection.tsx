...
export default function SkillsSection() {
  ...
  return (
    <div className="flex overflow-hidden w-full flex-col py-12 md:py-16 px-6 md:px-20 bg-white antialiased text-xs max-w-7xl mx-auto relative">
      
      {/* Header Block (Matches ServicesSection exactly, fully mobile responsive) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full z-10 relative">
        {categories.map((category, catIndex) => {
          ...
          return (
            <div 
              key={category.title} 
              className={`flex flex-col p-6 md:p-10 border border-[#111111] hover:scale-[1.01] transition-transform duration-300 min-h-[380px] sm:min-h-[420px] lg:min-h-[500px] ${
                isBlackCard ? 'bg-[#111111] text-white' : 'bg-white text-[#111111]'
              }`}
            >
              {/* Category Header */}
              <div 
                className={`flex justify-between items-center w-full pb-4 md:pb-5 border-b ${
              ...
              >
                ...
                <span className={`text-xs md:text-sm font-extrabold ${isBlackCard ? 'text-[#666666]' : 'text-[#999999]'}`}>
                  {category.id}
                </span>
              </div>
